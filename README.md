...
```powershell
python inference/predict_folder.py --model crossearth --input-dir data/raw/planetscope/images --sensor-config configs/sensors/planetscope.yaml --checkpoint outputs/checkpoints/crossearth_lightning/best.ckpt --output-dir outputs/predictions/crossearth/planetscope --device cuda:0 --recursive --skip-existing
```
---